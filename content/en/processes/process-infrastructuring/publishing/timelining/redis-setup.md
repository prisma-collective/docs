...
## Automatic provisioning via Propagate

When deploying timelining with propagate, Redis is **provisioned automatically** during `propagate apply` unless you override credentials in `.propagate/values.yaml`:

1. First Pulumi pass deploys apps without Redis (Vercel projects must exist first)
2. CLI verifies the Upstash marketplace integration on your Vercel team (`propagate auth upstash`)
3. Second Pulumi pass creates the Redis store, connects it to timelining, and redeploys so `KV_REST_*` env vars are live

Prerequisites: complete `propagate auth vercel` with marketplace/storage token permissions. See [Propagate CLI](/processes/process-infrastructuring/propagate/cli) and [Workflow](/processes/process-infrastructuring/propagate/workflow).

To skip auto-provisioning, add `upstash.kv` to `.propagate/values.yaml` or follow the manual steps below.
