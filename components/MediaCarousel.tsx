...
type MediaCarouselProps = {
  ...
  videos?: string[];
  alt?: string | string[];
  /** Show border around the media frame. Defaults to true. */
  border?: boolean;
};
...
export default function MediaCarousel({ images, videos, alt, border = true }: MediaCarouselProps) {
  ...
  return (
    <div className={`relative flex w-full max-w-none flex-col gap-3 ${mode === 'video' ? 'not-prose' : ''}`}>
      <div
        className={`relative w-full min-w-0 max-w-none overflow-hidden rounded-2xl bg-black/30 ${
          border ? 'border border-prisma-b' : ''
